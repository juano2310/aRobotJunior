	...
	def SetSpeedFromLines(self, lines, buildDisplay, displayImage):
		...
		if len(bestLine) < 3:
			...
			if buildDisplay:
				...
				cv2.line(displayImage, centralPoint, dPoint, lineColour, 3, lineType = cv2.LINE_AA) 
				cv2.line(displayImage, d2PointA, d2PointB, d2Colour, 3, lineType = cv2.LINE_AA) 
				cv2.line(displayImage, offsetPoint, centralPoint, white, 1) 
				cv2.line(displayImage, centralPoint, dPoint, white, 1, lineType = cv2.LINE_AA) 
				cv2.line(displayImage, d2PointA, d2PointB, white, 1, lineType = cv2.LINE_AA) 
				# Offset plot
				d0Sum = dLine + d0
				...
				for line in trackParts:
					x1 = section * line[0]
					x2 = x1 + section
					cv2.line(displayImage, (x1, y), (x2, y), line[1], fat, lineType = cv2.LINE_AA)
				y1 = y - Settings.offsetHeight
				y2 = y + Settings.offsetHeight
				x = int(section * (4 - d0Sum))
				cv2.line(displayImage, (x, y1), (x, y2), black, fat, lineType = cv2.LINE_AA)
				cv2.line(displayImage, (x, y1), (x, y2), orange, thin, lineType = cv2.LINE_AA)
				# Overtaking status
				font = cv2.FONT_HERSHEY_SIMPLEX
				if Globals.controller.overtaking:
					text = 'Overtaking: %+.1f' % (Globals.controller.autoTargetLane)
					cv2.putText(displayImage, text, (5, 20), font, 0.25, black, 1, cv2.LINE_AA)
		# Display status
		if (Globals.frameAnnounce == 0):
